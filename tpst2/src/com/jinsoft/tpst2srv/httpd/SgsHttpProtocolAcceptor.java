...
package com.jinsoft.tpst2srv.httpd;
...
public class SgsHttpProtocolAcceptor extends AbstractService implements ProtocolAcceptor {
	/** The package name. */
	private static final String PKG_NAME = "com.jinsoft.tpst2srv.httpd";
	
	/** The logger for this class. */
	private static final LoggerWrapper LOG = new LoggerWrapper(Logger.getLogger(PKG_NAME + ".acceptor"));
	 ...
	 */
	private final ConcurrentSkipListMap<Long, SessionProtocol> disconnectingHandlersMap = new ConcurrentSkipListMap<>();
	...
	private final Map<String, HttpClientSession> activeSessions = new ConcurrentHashMap<>();
	...
	public SgsHttpProtocolAcceptor(Properties properties, ComponentRegistry systemRegistry, TransactionProxy txnProxy) throws Exception {
		this(properties, systemRegistry, txnProxy, LOG);
                LOG.log(Level.CONFIG, "{0}:{1}:{2}:{3}", new Object[]{properties, systemRegistry, txnProxy, LOG});
	}
