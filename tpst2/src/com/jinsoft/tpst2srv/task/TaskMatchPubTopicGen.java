 ...
 * @author KeyuGG
 */
public class TaskMatchPubTopicGen implements Serializable, ManagedObject, Task {

    private static final long serialVersionUID = 1L;
    private static final Logger LOG = Logger.getLogger( TaskMatchPubTopicGen.class.getName() );
    ...
    public TaskMatchPubTopicGen( int fmatchid ){
    ...
    public void run() throws Exception {
        ...
        try
        {
            sql = String.format( GameData.QUERY_GAME_CULLTOPIC, 1, Global.DATAFORMAT_HM.format(GameData.matchTime.get("emucurrTime")) );
            //LOG.log(Level.INFO, "生成公共题的查询: {0}", sql);
            GameQuery.query( sql, new GameResultHandler( GameMessage.GAME_CULL_TOPIC, null ) );
   
        } catch ( Exception e ){
            LOG.log(Level.INFO, "生成公共题的查询时出错: {0}", e.getMessage());
        }
        //queryData();
        //sendResponseData();
